...
class IOCOptimizer:
    ...
    def _solve_ipopt(
        ...
        g_expr      = ca.vertcat(csc_expr, r_norm_expr)   # (e+1, 1)
        ...
        ipopt_opts = {
            'ipopt.max_iter'             : self.cfg.get('max_iterations'),
            'ipopt.tol'                  : self.cfg.get('tol', 1e-6),
        ...
        # Optionally suppress CasADi / IPOPT native stdout/stderr (C/C++ level)
        # Set 'suppress_casadi_output' in optimizer config to True to enable.
        # 주의: 이 모드에서는 스피너 출력도 함께 숨겨질 수 있습니다.
        spinner = None
        if self.cfg.get('suppress_casadi_output', False):
            ...
            try:
                ...
                sol = solver_fn(x0=x0, lbx=lbw, ubx=ubw, lbg=lbg, ubg=ubg)
            finally:
                # Restore fds
